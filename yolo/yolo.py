...
class YOLO(object):
    ...
    def _generate(self):
        ...
        # Cargando el modelo, construyendolo
        num_anchors = len(self.anchors)
        num_classes = len(self.class_names)
        try:
            self.yolo_model = load_model(model_path, compile=False)
        except:
            # Ancho y clases del modelo
    ...
    def detect_image(self, image):
        ...
        print('*** Encontrado {} Rostro(s) para la imagen'.format(len(out_boxes)))
        ...
        end_time = timer()
        print('*** Tiempo proceso: {:.2f}ms'.format((end_time -
                                                          start_time) * 1000))
...
def detect_video(model, video_path=None, output=None):
    ...
    if not vid.isOpened():
        raise IOError("Error camara o video")
